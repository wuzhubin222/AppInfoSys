var path = $("#path").val();  //得到id="path"的隐藏域里面的值，即/项目名
$("#queryCategoryLevel1").change(function(){
	var queryCategoryLevel1 = $("#queryCategoryLevel1").val();
	if(queryCategoryLevel1 != '' && queryCategoryLevel1 != null){
		$.ajax({
			type:"GET",//请求类型
			url:path +"/dev/appinfo/queryCategoryListByParentId",//请求的url
...
$(".addVersion").on("click",function(){
	var obj = $(this);
	window.location.href= path + "/dev/appinfo/appversionadd/"+obj.attr("appinfoid");
});
//[修改版本]的点击事件
$(".modifyVersion").on("click",function(){
	...
	if(status == "1" || status == "3"){//待审核、审核未通过状态下才可以进行修改操作
		if(versionid == null || versionid == ""){
			...
			window.location.href= path + "/dev/appinfo/appversionmodify?vid="+ versionid + "&aid="+ appinfoid;
		}
	}else{
		alert("该APP应用的状态为：【"+obj.attr("statusname")+"】,不能修改其版本信息，只可进行【新增版本】操作！");
	}
});
//点击[修改]选择跳转到Controller中
$(".modifyAppInfo").on("click",function(){
	var obj = $(this);
	var status = obj.attr("status");
	if(status == "1" || status == "3"){//待审核、审核未通过状态下才可以进行修改操作
		window.location.href = path + "/dev/appinfo/appinfomodify/"+ obj.attr("appinfoid");
	}else{
