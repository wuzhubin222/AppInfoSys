...
var path = $("#path").val();  /*<input type="hidden" id="path" />  得到：/项目名*/
...
function delfile(id){
	$.ajax({
		...
		url : path + "/dev/appinfo/delfile",
		data:{id:id,flag:'logo'},//请求参数
		dataType:"json",//ajax接口（请求url）返回的数据类型
		success:function(data){//data：返回数据（json对象）
			if(data.result == "success"){
				alert("删除成功！");
				$("#uploadfile").show();  //将图片上传Html代码给显示出来。
				$("#logoFile").html(''); //将显示图片logo的html代码给隐藏。
			}else if(data.result == "failed"){
				alert("删除失败！");
			} else if (data.result == "noexist") {
				alert("要删除的物理文件在服务器中不存在！");
			}
		},
		error:function(data){//当访问时候，404，500 等非200的错误状态码
			alert("请求错误！");
...
$(function(){  
	...
	$.ajax({
		...
		url : path + "/dev/appinfo/getDataDictionaryByCode",
